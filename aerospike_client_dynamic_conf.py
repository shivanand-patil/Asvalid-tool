...
# Define Aerospike server configuration
config = {
        'hosts': [('localhost', 3000)]  # Change host and port if necessary
}

# Connect to Aerospike server
try:
    client = aerospike.client(config).connect()
    print("Connected to Aerospike server successfully")
except Exception as e:
    print(f"Failed to connect to Aerospike server: {e}")
    exit()
